...
class Unwarp:
    ...
    def inference(self, image: Image.Image) -> Image.Image:
        ...
        # Ensure `img_size` is int64 as required by ONNX
        unwarped = self.bilinear_unwarping.run(
            None,
            {
                "warped_img": original_input.astype(np.float32),
                "point_positions": points.astype(np.float32),
                "img_size": np.array(original_size, dtype=np.int64),  # Ensure int64 here
            },
        )[0][0]

        # Convert the output array to an image format
